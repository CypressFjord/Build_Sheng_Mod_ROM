...
#移植包系统OS版本号
port_os_version=$(echo "$SYSTEM_URL" | awk -F'/' '{print $(NF-1)}')
#移植包系统zip名称
port_zip_name=$(echo "$SYSTEM_URL" | awk -F'/' '{print $NF}' | awk -F'?' '{print $1}')
#Android版本号
android_version=$(echo "$SYSTEM_URL" | grep -oE '-user-[0-9]+' | grep -oE '[0-9]+')
#构建时间
...
######构建$device移植ROM
echo -e "${Red}- 开始构建$device移植ROM${NC}"
Start_Time

###底包和移植包下载
echo -e "${Red}- 开始底包和移植包下载${NC}"
Start_Time
echo -e "${Red}- 开始底包下载${NC}"
Start_Time
if [ -f "$GITHUB_WORKSPACE/${vendor_zip_name}" ]; then
    echo -e "${Green}- 检测到本地已存在底包: ${vendor_zip_name}，跳过网络下载步骤${NC}"
else
    echo -e "${Yellow}- 本地不存在底包，开始网络下载${NC}"
    aria2c -x16 -s16 -j$(nproc) -U "Mozilla/5.0" -d "$GITHUB_WORKSPACE" ${VENDOR_URL} &
    wait
fi
End_Time 底包下载
echo -e "${Red}- 开始移植包下载${NC}"
Start_Time
if [ -f "$GITHUB_WORKSPACE/${port_zip_name}" ]; then
    echo -e "${Green}- 检测到本地已存在移植包: ${port_zip_name}，跳过网络下载步骤${NC}"
else
    echo -e "${Yellow}- 本地不存在移植包，开始网络下载${NC}"
    aria2c -x16 -s16 -j$(nproc) -U "Mozilla/5.0" -d "$GITHUB_WORKSPACE" ${SYSTEM_URL} &
    wait
fi
End_Time 移植包下载
End_Time 底包和移植包下载
...
###解压并分解底包和移植包的文件
echo -e "${Red}- 开始解压并分解底包和移植包的文件${NC}"
Start_Time
...
$payload_extract -s -o "$GITHUB_WORKSPACE"/Extra_dir -i "$GITHUB_WORKSPACE"/vendor_zip/payload.bin -X odm,system_dlkm,vendor,vendor_dlkm -T0
sudo rm -rf "$GITHUB_WORKSPACE"/vendor_zip/payload.bin
End_Time 分解底包Payload
#解压移植包ZIP
echo -e "${Yellow}- 开始解压移植包ZIP${NC}"
Start_Time
...
End_Time 分解移植包Payload
#分解底包和移植包的Images
echo -e "${Red}- 开始分解底包和移植包的Images${NC}"
Start_Time
...
End_Time 分解底包和移植包的Images
End_Time 解压并分解底包和移植包的文件
###解压并分解底包和移植包的文件结束

###写入底包和移植包的变量
echo -e "${Red}- 开始写入底包和移植包的变量${NC}"
Start_Time
#ROM构建日期
echo -e "${Red}- ROM构建日期: $build_time${NC}"
echo "build_time=$build_time" >>$GITHUB_ENV
#底包系统版本
echo -e "${Red}- 底包系统版本: $vendor_os_version${NC}"
...
echo -e "${Red}- 底包Vendor安全补丁日期: $vendor_security_patch${NC}"
...
echo -e "${Red}- 移植包SOTA版本: $incremental_version${NC}"
echo "incremental_version=$incremental_version" >>$GITHUB_ENV
#移植包系统版本
echo -e "${Red}- 移植包系统版本: $port_os_version${NC}"
...
echo -e "${Red}- 移植包System安全补丁日期: $port_security_patch${NC}"
...
echo -e "${Red}- 移植包System基线版本: $port_base_line${NC}"
echo "port_base_line=$port_base_line" >>$GITHUB_ENV
End_Time 写入底包和移植包的变量
###写入底包和移植包的变量结束
...
add_module_load "android13-5.15_moon_vmscan.ko"
...
Start_Time
sudo $a7z a "$GITHUB_WORKSPACE"/zip/Sheng-Mod-HyperOS-${model}-${port_os_version}-CypressFjord.zip "$GITHUB_WORKSPACE"/firmware/* >/dev/null
sudo rm -rf "$GITHUB_WORKSPACE"/images
End_Time 生成ZIP刷机包
#定制ROM包名
echo -e "${Red}- 开始定制ROM包名${NC}"
Start_Time
md5=$(md5sum "$GITHUB_WORKSPACE"/zip/Sheng-Mod-HyperOS-${model}-${port_os_version}-CypressFjord.zip)
echo "MD5=${md5:0:32}" >>$GITHUB_ENV
zip_md5=${md5:0:10}
rom_name="Sheng-Mod-HyperOS-${model}-${port_os_version}-CypressFjord-${zip_md5}.zip"
sudo mv "$GITHUB_WORKSPACE"/zip/Sheng-Mod-HyperOS-${model}-${port_os_version}-CypressFjord.zip "$GITHUB_WORKSPACE"/zip/"${rom_name}"
echo "rom_name=$rom_name" >>$GITHUB_ENV
End_Time 定制ROM包名
End_Time 生成完整刷机包
###生成完整刷机包结束

End_Time 构建$device移植ROM
######构建$device移植ROM结束
